...
if (typeof window !== 'undefined') {
  console.log('🔑 Purchase Service Loaded (Matches Deployed Rust Program)');
...
// ============================================================================
// ✅ FIXED: Get authority (LINE 177 FIX)
// ============================================================================
/**
 * Get authority - uses the hardcoded authority from config
 * The authority never changes after presale initialization
 * This eliminates the need for async account fetching and improves reliability
 */
const getAuthority = (): PublicKey => {
  return BLOCKCHAIN_CONFIG.authority;
};
...
export const buildSOLPurchaseTransaction = async (
  ...
  try {
    ...
    // ────────────────────────────────────────────────
    // 1. Get authority (FIXED - no longer async)
    // ────────────────────────────────────────────────
    ...
    console.log('✅ All 14 accounts prepared for SOL purchase');
    ...
    console.log('✅ SOL purchase transaction built successfully');
    ...
    console.error('❌ Error building SOL purchase transaction:', error);
    return {
      success: false,
...
export const buildUSDCPurchaseTransaction = async (
  ...
  try {
    ...
    // ✅ FIXED: Use simple authority getter
    const authority = getAuthority();
    ...
    const [buyerDataPda] = getBuyerDataPDA(userPublicKey);
    const [presaleAuthorityPda] = getPresaleAuthorityPDA(authority);
    const [presaleUsdcVaultPda] = getUsdcVaultPDA(authority);
    const [tokenVaultPda] = getTokenVaultPDA(authority);

    console.log('  USDC Vault PDA:', presaleUsdcVaultPda.toString());
    ...
    console.log('✅ USDC purchase transaction built successfully');
    ...
    console.error('❌ Error building USDC purchase transaction:', error);
...
export const buildUSDTPurchaseTransaction = async (
  ...
  try {
    ...
    console.log('✅ USDT purchase transaction built successfully');
    ...
    console.error('❌ Error building USDT purchase transaction:', error);
    return { success: false, error: error.message || 'Unknown error' };
  }
};
  ...
  try {
    ...
    if (error.message) {
      ...
      if (msg.includes('custom program error: 0x1')) {
        userMessage = 'Insufficient balance or presale not active';
      }
...
// ============================================================================
// HELPER: Get current phase and price
// ============================================================================
