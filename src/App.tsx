...
import ProfitDistributionSection from './components/sections/ProfitDistributionSection';
...
const App = () => {
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // WALLET & CONNECTION - ✅ CRITICAL FIX: Added useAppKitProvider
  // ──────────────────────────────────────────────────────────────────────────
  const { open, disconnect } = useAppKit();
  const { address, isConnected } = useAppKitAccount();
  const { walletProvider } = useAppKitProvider('solana'); // ✅ THIS IS THE KEY FIX!
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // STATE: WALLET COMPATIBILITY
  // ──────────────────────────────────────────────────────────────────────────
  const [walletCompatibility, setWalletCompatibility] = useState<{
  ...
  const fetchBalances = async (showLoadingIndicator = true) => {
    ...
    try {
      ...
      console.log('✅ Balances fetched:', result.balances);
  ...
  const calculateTokenAmount = () => {
    ...
    const totalTokens = baseTokens + bonusTokens;
      console.log('💰 Token calculation:', { usdValue, price, baseTokens, bonusPercentage, bonusTokens, totalTokens, totalTokensWithDecimals: totalTokens * 1e9 });
  ...
  const buyTokens = async () => {
    ...
    // ✅ CHECK WALLET COMPATIBILITY - Prevent MetaMask and other Ethereum wallets
    if (walletCompatibility && !walletCompatibility.isCompatible) {
      ...
      showNotification(
        'error',
        lang === 'ar'
          ? `${walletCompatibility.walletName} غير متوافقة. استخدم: ${walletList}`
          : `${walletCompatibility.walletName} is incompatible. Please use: ${walletList}`
      );
      
      // Open wallet modal to allow user to switch
    ...
    try {
      ...
      if (!rateLimitCheck.allowed) {
        showNotification(
          'error',
          lang === 'ar'
            ? `يرجى الانتظار ${rateLimitCheck.retryAfter} ثانية`
    ...
    try {
      ...
      // ========== VIP MINIMUM CHECK (AFTER USD CONVERSION) ==========
      if (isVipPeriod && usdValue < 1000) {
      ...
      // ========== BUILD TRANSACTION ==========
      showNotification('info', lang === 'ar' ? 'جاري بناء المعاملة...' : 'Building transaction...');
      ...
      if (currency === 'SOL') {
        console.log('🟣 Building SOL purchase transaction...');
        ...
        console.log('🔵 Building USDC purchase transaction...');
        ...
        console.log('🟢 Building USDT purchase transaction...');
      ...
      const isWalletConnected = provider.isConnected || false;
      console.log('📊 Wallet isConnected:', isWalletConnected);

      if (!isWalletConnected && provider.connect && typeof provider.connect === 'function') {
        ...
        try {
          ...
          throw new Error('Please approve the wallet connection to continue');
        }
      }
      ...
      showNotification(
        'success',
        `${lang === 'ar' ? 'نجح الشراء!' : 'Purchase successful!'} ${tokensReceived.toLocaleString()} LLTY${bonusPercentage > 0 ? ` (+${bonusPercentage}% bonus)` : ''}`
      );
      ...
      if (errorMessage.includes('User rejected')) {
        ...
        errorMessage = lang === 'ar' ? 'يرجى إعادة توصيل المحفظة' : 'Please reconnect your wallet';
      }
  ...
  useEffect(() => {
    if (isConnected && walletProvider) {
      ...
      setWalletCompatibility(compatibilityInfo);
      
      // Show warning if wallet is incompatible
      if (!compatibilityInfo.isCompatible) {
        showNotification(
          'error',
          lang === 'ar' 
            ? `${compatibilityInfo.walletName} غير متوافقة مع سولانا. استخدم Phantom أو Solflare`
            : `${compatibilityInfo.walletName} is not compatible with Solana. Use Phantom or Solflare`
        );
      }
    } else {
      setWalletCompatibility(null);
    }
  }, [isConnected, walletProvider, address, lang]);
